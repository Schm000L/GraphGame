...
function deltas(edge:Edge):[number, number] {
    ...
    let deltaY:number = Math.abs(center1[0] - center2[0])
    let deltaX:number = Math.abs(center1[1] - center2[1])
...
export function edgeParameters(edge:Edge):[number, number, number, number] {
    let [deltaY, deltaX] = deltas(edge)
    return [deltaY/2, deltaX/2, Math.sqrt(deltaY**2+deltaX**2), deltaX!==0 ? Math.atan(deltaY/deltaX) : 0]
}